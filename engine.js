...
function markup(mount){
  mount.innerHTML=`<div class="kp-game"><div class="kp-world"></div><div class="kp-stars"></div><div class="kp-decor"></div><div class="kp-vignette"></div><header class="kp-topbar"><div class="kp-mission"><small>ТЕКУЩАЯ КОМАНДА</small><strong class="missionText">Загрузка…</strong><span class="groupCount"></span></div><div class="kp-score"><div class="kp-pill"><b class="scoreVal">0</b><span>очков</span></div><div class="kp-pill"><b class="errVal">0</b><span>ошибок</span></div></div></header><div class="kp-toast" role="status"></div><div class="kp-field"></div><div class="kp-beam"></div><div class="kp-vacuum"><img src="assets/vacuum.png" alt="Космический пылесос"></div><div class="kp-controls"><label class="themeControl">Тема <select class="themeSelect" aria-label="Выберите тему"><option value="parts-of-speech-demo">Части речи</option><option value="participle-suffixes">Суффиксы причастий</option></select></label><label class="speedControl">Скорость <input class="speedRange" type="range" min="0.35" max="2.5" step="0.05"></label><button class="kp-btn grabBtn">Захват</button><button class="kp-btn soundBtn">Звук: вкл.</button><button class="kp-btn repairBtn">Ремонтный отсек</button><button class="kp-btn cherry resetBtn">Сбросить</button></div><div class="kp-modal"><div class="kp-panel"><h2>Ремонтный отсек</h2><p class="sub">Исправь ошибки полёта. Регистр и знак вопроса не мешают проверке.</p><div class="kp-summary"></div><div class="repairList"></div><div class="panelActions"><button class="kp-btn closeRepair">Вернуться в полёт</button></div></div></div></div>`;
...
function start(config,opts={}){
  ...
  const state={groupIndex:0,items:[],errors:[],score:0,speed:clamp(+config.speed||.75,.35,2.5),pointer:{x:innerWidth*.52,y:innerHeight*.58},vac:{x:innerWidth*.52,y:innerHeight*.58},facing:-1,cameraX:0,last:performance.now(),running:true,active:false,muted:false,finished:false,decor:[]};
  try{const saved=JSON.parse(localStorage.getItem(key)||'null');if(saved)Object.assign(state,saved)}catch(e){}
  state.items=[];state.decor=[];state.groupIndex=clamp(+state.groupIndex||0,0,config.groups.length-1);state.speed=clamp(+state.speed||.75,.35,2.5);
  ...
  function tick(t){
    ...
    const speedFactor=state.speed/.72;state.vac.x+=(state.pointer.x-state.vac.x)*Math.min(1,dt*8*speedFactor);state.vac.y+=(state.pointer.y-state.vac.y)*Math.min(1,dt*8*speedFactor);
